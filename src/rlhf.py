...
def _vdel(keytag):
    ...
    #st.session_state[KP + 'labeler_completed'] = "None"
...
def display_chat_turn_box(keytag, label, idx, message):
    if _vcheck(KP + "chat_display_mode", "rich"):
        ...
        st.text_area(label=label, value=message, on_change=update_chat, args=(idx, keytag),
                     key=keytag, height=max(300, min(600, int(len(message) / 3))),help="Click to update the chat")

######## Anirudh ############

def similarity_check(input_str, str_list):
    threshold = int(st.session_state[KP + 'similarity_threshold'])
    results = []
    max_similarity = {'input_str': input_str, 'item': '', 'similarity_score': 0} 
    for item in str_list:
        similarity_score = fuzz.ratio(input_str.lower(), item.lower())
        if similarity_score > threshold:
            overlap_data = {'input_str': input_str, 'item': item, 'similarity_score': similarity_score}
            results.append(overlap_data)
            max_similarity = max(results, key=lambda x: x['similarity_score'])
    #print("max_similarity and threshold = ", max_similarity['similarity_score'], threshold)
    return max_similarity['similarity_score'] > threshold, results


# Function to read question bank data from JSON file
def read_question_bank():
    if KP + 'question_bank_file' in st.session_state:  
        try:
            with open(st.session_state[KP + "question_bank_file"], "r") as file:
...
def write_question_bank(question_bank):
    with open(st.session_state[KP + "question_bank_file"], "w") as file:
        json.dump(question_bank, file, indent=2)


def search_question_bank(keytag):
    # st.session_state[KP + 'question_exists'] = False
     # Read question bank data
    st.session_state[KP + 'question_bank'] = read_question_bank()
    # Search for a question and remove exceeding and trailing spaces
    st.session_state[KP + 'question_search_query'] = st.session_state[keytag].strip()
    if st.session_state[KP + 'question_search_query']:
        #check for similarity questions
        st.session_state[KP + 'question_is_similar'], st.session_state[KP + 'question_overlap_data'] = similarity_check(st.session_state[KP + 'question_search_query'], st.session_state[KP + 'question_bank'])
        print(st.session_state[KP + 'question_is_similar'], st.session_state[KP + 'question_overlap_data'])
    
def add_question_to_bank(click):
    if KP + 'question_search_query' in st.session_state:
        if click and st.session_state[KP + 'question_search_query'] not in st.session_state[KP + 'question_bank']:
                st.session_state[KP + 'question_bank'].append(st.session_state[KP + 'question_search_query'])
                write_question_bank(st.session_state[KP +'question_bank'])
                return "Question added to the bank"
        else:
            return "Overwrite"
    return "recheck"
...
def display_chat_user_context(idx, message, role, edit_display):
    ...
    if role == "context":
        ...
        if edit_display:
            ...
            c1.button(":x:", on_click=delete_item, args=(idx,), key=KP + str(idx) + 'c1', help="Click to delete prompt")
            c2.button(":heavy_plus_sign:", on_click=add_item, args=(idx, role,), key=KP + str(idx) + 'c2', help="Click to add prompt")
            if _vexists(KP + "tags"):
                with c3:
            ...
            if idx > first_user_msg:
                c4.button(":arrow_up:", on_click=move_up, args=(idx,), key=KP + str(idx) + 'c4',help="Click to move the chat up")
            if idx < len(st.session_state[KP + 'wset']) - 1:
                c5.button(":arrow_down:", on_click=move_down, args=(idx,), key=KP + str(idx) + 'c5', help="Click to move the chat down")
    ...
    def add_robot_box_buttons(idx, content_type):
        ...
        if _vtrue(KP + "chatbox_delete_confirmation"):
            if _vtrue(KP + str(idx) + 'cx' + content_type):
                cx.button("Sure :boom:", on_click=delete_item, args=(idx,), key=KP + str(idx) + 'cx.2' + content_type,help="Click to delete")
            else:
                cx.button(":x:", key=KP + str(idx) + 'cx' + content_type,help="Click to delete")
        else:
            cx.button(":x:", on_click=delete_item, args=(idx,), key=KP + str(idx) + 'cx' + content_type,help="Click to delete")
        ...
        if idx > 0 and st.session_state[KP + 'wset'][idx]['model'] in predict.get_models():
            c7.button(":robot_face:", on_click=gen_message,
                      ...
                      key=KP + str(idx) + 'c7' + content_type,help="Click to regenerate response")
        ...
        with mid:
            ...
            if _vtrue(KP + "add_agent_duo_regenerate_button"):
                st.button(":robot_face:", on_click=gen_message,
                          ...
                          key=KP + "Duo_Regenerate" + str(idx) + "both",help="Click to regenerate both responses")
    else:
...
def get_chatstorefilepath():
    if st.session_state[KP + "chat_store"] == '':
        ...
        if KP + "suggested_chat_label" in st.session_state and st.session_state[KP + "suggested_chat_label"]:
            print("INSIDE IF ", st.session_state[KP + "chat_label"])
...
def unmark_completed():
    _vdel(KP + 'question_is_similar')
    st.session_state[KP + 'wset'] = [x for x in st.session_state[KP + 'wset'] if x['role'] != 'labeler_completed']
    st.session_state[KP + 'labeler_completed'] = "None"
    completed_file = os.path.join(st.session_state[KP + "completed_dir"], os.path.basename(get_chatstorefilepath()))
...
def show_chat():
    ...
    def display_chat_add_context_box():
        if _vexists(KP + "add_context_submit_button"):
            ...
            if _vcheck(KP + "chat_display_mode", "rich"):
                ...
                st.text_area("Context", key=KP + "context_msg",help="Enter context")
            st.button("Submit", on_click=add_context_in_chat, key=KP + "context_add_key",help="Click to add context")
        else:
            if _vexists(KP + "allow_context_label_entry"):
                st.text_input("Chat Label", key=KP + "suggested_chat_label")
            if _vcheck(KP + "chat_display_mode", "rich"):
                ...
                st.text_area("Context", key=KP + "context_msg", on_change=add_context_in_chat, args=(0,),help="Enter context")
    ...
    def prompt_provided(keytag):
        search_question_bank(keytag)
        add_chat("prompt", st.session_state[keytag], False, '')
        
    def reviewer_comments_provided(keytag1):
        add_chat("reviewer_comments", st.session_state[keytag1], False, '')
        add_chat("review_check", "Yes", False, '')
        
    def labeler_completion_selection(keytag2):
        if st.session_state[keytag2] != "None":
            add_chat("labeler_completed", st.session_state[keytag2], False, '')
        else:
            # remove this role from the chat
            st.session_state[KP + 'wset'] = [x for x in st.session_state[KP + 'wset'] if x['role'] != 'labeler_completed']
    ...
    if is_completed():
        ...
        coll.button("Revert Completed", on_click=unmark_completed,help="Click to revert the task from completed state")
        ...
        if _vcheck(KP + "chat_display_mode", "rich"):
            ...
            s1.text_area("Prompt here", key=KP + main_msg_key, value=main_msg, on_change=prompt_provided,
                         args=(KP + main_msg_key,),help="Enter the prompt question here and hit CMD+Enter or CLT+Enter to check the question in the question bank")
            s2.write("Add or override this question to the question bank?",unsafe_allow_html=True)
            # fit to column width
            if  s2.button("Yes 📌 ",key=KP+"question_add",help="Click to add question to question bank",use_container_width=True):
                success = add_question_to_bank(True)
                print("Success = ", success)
                if success == 'Question added to the bank':
                    st .success('Successfully added the question to the bank', icon="✅")
                if success == 'Overwrite':
                    st.success('Successfully Overwritten to the question to the bank', icon="✅")
                
 
        if _vtrue(KP + 'question_is_similar'):
            st.write(st.session_state[KP + 'question_overlap_data'])
            st.warning('Warning (This question already exists in the Question bank)', icon="⚠️")
        elif not _vtrue(KP + 'question_is_similar') and KP + 'question_is_similar' in st.session_state:
            st.success('No matching questions in question bank', icon="👍")                

        st.button("Submit to Model", on_click=prompt_submit, args=("user", KP + main_msg_key, True,),
                    key=KP + "content_add_key", help="Click to submit the prompt to the model")
        
        col1,col2,col3 = st.columns([2,2,1])
        col1.selectbox('Labeler mark complete', options=["None", "Completed by labeler"], key=KP + "labeler_completed", on_change=labeler_completion_selection, args=(KP + "labeler_completed",),help="Select to mark the task as Completed by labeler")
        
        
        st.divider()
        st.subheader("Reviewer Section")
        # review section
        riv1,riv2, riv3 = st.columns([1,3,1])
        riv1.checkbox("Review", key=KP + "review_check", value = False,help="Check to review the task")
        if _vtrue(KP + "review_check"):
            riv2.text_area("Reviewer comments", key= KP + "reviewer_comments", value="", on_change=reviewer_comments_provided, args=(KP + "reviewer_comments",),help="Comments from the reviewer")
        riv3.button("Reviewer Mark Completed", on_click=mark_completed,
                    disabled=len(st.session_state[KP + "wset"]) == 0, help="Click to mark as completed (Only by the reviewer)")
...
def load_context_folders():
    ...
    st.session_state[KP + "contexts"] = get_file_and_content_map_from_folder(st.session_state[KP + "context_folders"],
                                                                             _vtrue(
                                                                                 KP +"add_context_additional_info_in_chat"))
...
def init(force=False, info=""):
    ...
    st.session_state[KP + 'question_bank'] = read_question_bank()
    st.session_state[KP + 'review_check'] = False
                ...
                if os.path.isfile(tag_fpath):
                    ...
                    for turn in turns:
                        if turn["role"] == "context":
                            ...
                            response_count += 1
                        elif turn['role'] == 'reviewer_comments':
                            tdata['reviewer_comments'] = turn['content']
                        elif turn['role'] == 'review_check':
                            tdata['review_check'] = 'Yes'
                        elif turn['role'] == 'labeler_completed':
                            tdata['labeler_completed'] = turn['content']
...
def sidebar():
    ...
    def chat_new():
        ...
        st.session_state[KP + "chat_store"] = ""
        st.session_state[KP+ 'labeler_completed'] = "None"
        st.session_state[KP +'success'] = False
        _vdel(KP + 'loaded_object')
        _vdel(KP + 'add_context')
        st.session_state[KP + 'question_overlap_data']=''
        st.session_state[KP + "model"] = get_default_chat_model()
        # if KP + 'question_add' in st.session_state:
        #     st.session_state[KP + 'question_add'] = "No"
        _vdel(KP + 'question_is_similar')
    ...
    def load_chat():
        ...
        for d in st.session_state[KP + 'wset']:
            if  d['role'] == 'labeler_completed':
                st.warning("labeler Marked Completed",icon="🔥")
                break
    ...
    def show_load_chat_box():
        ...
        for chat in chat_list:
            ...
            # if st.session_state[KP + "labeler_completed"] != "None":
            #         chat_displayname[chat] += "[L-CMP]"
...
def clean_project_space():
    ...
    for key in st.session_state:
        for entry in ["tags_", "cmptask_","question_"]:
